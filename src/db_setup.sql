...
CREATE TABLE Propietario (
    ...
    propietario_persona_id         INT     NOT NULL     references Persona(persona_id), 
    propietario_empresa_id         INT     NOT NULL     references Empresa(empresa_id)
);
...
CREATE TABLE Cuenta (
    ...
    cuenta_propietario_id   INT     NOT NULL    references Propietario(propietario_id)
);

CREATE TABLE Carga(
    carga_id            INT         SERIAL      PRIMARY KEY,
    carga_cuenta_numero INT         NOT NULL    references Cuenta(cuenta_numero),
...
CREATE TABLE Vehiculo(
    ...
    vehiculo_id_rfid    INT             NOT NULL    UNIQUE, 
    vehiculo_tipo_id    INT             NOT NULL    references TipoVehiculo(tipo_vehiculo_id)
);

CREATE TABLE TipoVehiculo(
    tipo_vehiculo_id    INT         SERIAL      PRIMARY KEY,
    tipo_vehiculo_tipo  VARCHAR(35) NOT NULL    UNIQUE,
);

CREATE TABLE Tarifa(
    tarifa_id                   INT             SERIAL      PRIMARY KEY,
    tarifa_tipo_vehiculo_id     INT             NOT NULL    references TipoVehiculo(tipo_vehiculo_id),
...
CREATE TABLE Ventanilla(
    ventanilla_peaje_id         INT             NOT NULL    references Peaje(peaje_id),
...
CREATE TABLE Debito(
    ...
    debito_vehiculo_id          INT         NOT NULL    references Vehiculo(vehiculo_id),
    debioto_peaje_id            INT         NOT NULL    references Peaje(peaje_id),
    debito_ventanilla_numero    INT         NOT NULL    references Ventanilla(ventanilla_numero), 
    debito_fecha_y_hora         DATETIME    NOT NULL,
    debito_cuenta_numero        INT         NOT NULL    references Cuenta(cuenta_numero)
);


CREATE TABLE Bonificacion(
    bonificacion_id                 INT             SERIAL      PRIMARY KEY,
    bonificacion_cuenta_numero      INT             NOT NULL    references Cuenta(cuenta_numero), 
    bonificacion_fecha_otorgo       DATE            NOT NULL, 
    bonificacion_descuento          SMALLINT        NOT NULL, 
    bonificacion_motivo             VARCHAR(100)    NOT NULL,
    bonificacion_peaje_id           INT             NOT NULL    references Peaje(peaje_id), 
